  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center p-4">
      <div className={`w-full max-w-5xl overflow-hidden rounded-3xl shadow-2xl flex ${isMobile ? 'flex-col' : 'flex-row'} bg-white`}>
        <div className={`${isMobile ? 'order-2' : 'w-1/2'} ${isSignup && !isMobile ? 'order-2' : 'order-1'}`}>
        ...
        <div 
          className={`${isMobile ? 'order-1 py-16' : 'w-1/2'} ${isSignup && !isMobile ? 'order-1' : 'order-2'} 
            bg-gradient-to-br from-purple-600 to-indigo-700 flex flex-col items-center justify-center text-white p-8 relative overflow-hidden`}
        >
